...
import {
  ...
  StyledDiv,
  StyledLink,
} from "./CLMatchesElements";
import { useNavigate } from "react-router-dom";

const CLMatches = () => {
  const dispatch = useDispatch();
  const CLUpcommingMatches = useSelector(
    (state) => state.championsLeague.upcomingMatches
  );
  const CLUpcommingMatchesStatus = useSelector(
    (state) => state.championsLeague.upcomingMatchesStatus
  );
  const CLUpcommingMatchesError = useSelector(
    (state) => state.championsLeague.upcomingMatchesError
  );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          ...
                          />
                          <StyledLink
                            to={`/team/${getTeamId(match.homeTeam.name)}`}
                          >
                            {match.homeTeam.name}
                          </StyledLink>
                        </td>
                        <td>
                          ...
                          />
                          <StyledLink
                            to={`/team/${getTeamId(match.awayTeam.name)}`}
                          >
                            {match.awayTeam.name}
                          </StyledLink>
                        </td>
