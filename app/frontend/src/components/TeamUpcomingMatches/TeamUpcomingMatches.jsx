...
const TeamUpcomingMatches = () => {
  ...
  const TeamUpcomingMatches = useSelector(
    (state) => state.TeamUpcomingMatches.data
  );
  const TeamUpcomingMatchesStatus = useSelector(
    (state) => state.TeamUpcomingMatches.status
  );
              ...
              {matchesByDate[date].map((match) => (
                <tr key={match.id}>
                  <td>
                    ...
                    <StyledLink to={`/team/${match.home_team_id}`}>
                      {match.home_team}
                    </StyledLink>
                  </td>
                  <td>
                    <img src={match.away_team_crest} width={30}></img>
                    <StyledLink to={`/team/${match.away_team_id}`}>
                      {match.away_team}
                    </StyledLink>
                  </td>
                </tr>
